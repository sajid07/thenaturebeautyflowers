...
const CommercialEquipments = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        await fetchProduct({ category: 'commercial equipments' }); // Include category filter for Pool Lights
  ...
  const poolLightsProducts = products.filter((product) => product.category.toLowerCase() === 'commercial equipments');
  ...
  return (
    <>
      ...
      {loading ? ( // Show spinner when loading is true
        ...
        <h2 className="mb-4 text-center" style={{ color: '#FF5733' }}>Commercial Equipments</h2>
          <div className="row">
...
export default CommercialEquipments;
