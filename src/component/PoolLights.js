...
const PoolLights = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        await fetchProduct({ category: 'pool light' }); // Include category filter for Pool Lights
  ...
  const poolLightsProducts = products.filter((product) => product.category.toLowerCase() === 'pool light');
  ...
  return (
    <>
      ...
      {loading ? ( // Show spinner when loading is true
        ...
        <h2 className="mb-4 text-center" style={{ color: '#FF5733' }}>Pool Lights</h2>
          <div className="row">
