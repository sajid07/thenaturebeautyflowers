...
const PoolTiles = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        await fetchProduct({ category: 'pool tile' }); // Include category filter for Pool Lights
  ...
  const poolLightsProducts = products.filter((product) => product.category.toLowerCase() === 'pool tile');
  ...
  return (
    <>
      ...
      {loading ? ( // Show spinner when loading is true
        ...
        <h2 className="mb-4 text-center" style={{ color: '#FF5733' }}>Pool Tiles</h2>
          <div className="row">
