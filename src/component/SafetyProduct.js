...
const SafetyProducts = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        await fetchProduct({ category: 'safety product' }); // Include category filter for Pool Lights
  ...
  const poolLightsProducts = products.filter((product) => product.category.toLowerCase() === 'safety product');
  ...
  return (
    <>
      ...
      {loading ? ( // Show spinner when loading is true
        ...
        <h2 className="mb-4 text-center" style={{ color: '#FF5733' }}>Safety Products</h2>
          <div className="row">
