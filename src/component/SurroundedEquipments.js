...
const SurroundedEquipments = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        await fetchProduct({ category: 'surrounded equipment' }); // Include category filter for Pool Lights
  ...
  const poolLightsProducts = products.filter((product) => product.category.toLowerCase() === 'surrounded equipment');
  ...
  return (
    <>
      ...
      {loading ? ( // Show spinner when loading is true
        ...
        <h2 className="mb-4 text-center" style={{ color: '#FF5733' }}>Surrounded Equipments</h2>
          <div className="row">
...
export default SurroundedEquipments;
